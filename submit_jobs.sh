...
snakemake --executor slurm --slurm-logdir slurm_logs --default-resources slurm_partition=short --set-resources dada2_remove_chimeras_pe:slurm_partition=medium --configfile config/config.yaml --use-conda --conda-prefix=~/snakemake_conda_prefix --jobname {rulename}.{jobid} --jobs 6000 --keep-going --rerun-incomplete \
...
--set-resource-scopes cpus_per_task=local mem_mb=local --resources cpus_per_task=16 mem_mb=24000 --rerun-triggers mtime
